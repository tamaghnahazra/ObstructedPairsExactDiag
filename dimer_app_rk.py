...
class DimerApp:
    ...
    def compute_covering_graph(self):
        ...
        # Reverting temporal print check
        ...
        self.ax_graph.set_title(f"Covering Graph ({N_nodes} reachable states)")
        if N_nodes > 0:
            if self.render_mode == "3D":
                ...
                # Initial coloring (will be overridden by eigenvector color map)
        ...
        self.ax_mat.set_title("RK Hamiltonian")
        if N_nodes > 0:
            H_mat = np.copy(adj_mat)
            for i in range(N_nodes):
                state = idx_to_state[i]
                flips = self.get_flippable_plaquettes_of_state(state)
                H_mat[i, i] = len(flips)
                
            matrix_img = self.ax_mat.imshow(H_mat, cmap='Blues', interpolation='nearest')
    ...
    def save_matrix_pdf(self):
        """Export the RK Hamiltonian / Adjacency Matrix to a clean PDF."""
        # Calculate matrix if not done
        if len(self.states) == 0:
            print("Cannot export: compute covering graph first.")
            return
            
        # Re-derive matrix for high-quality export
        H_mat = np.zeros((len(self.states), len(self.states)))
        state_to_idx = {state: i for i, state in enumerate(self.states)}
        for u_fs, state in enumerate(self.states):
            adj = self.lattice.get_adjacent_states(list(state))
            for new_state in adj:
                new_state_fs = frozenset(new_state)
                if new_state_fs in state_to_idx:
                    v_fs = state_to_idx[new_state_fs]
                    H_mat[u_fs, v_fs] = 1

        fig = Figure(figsize=(8, 8))
        ax = fig.add_subplot(111)
        ax.imshow(H_mat, cmap='Blues', interpolation='nearest')
        ax.axis('off') # No title, no axes labels, no ticks
    ...
    def save_matrix_data(self):
        """Save the Hamiltonian/Adjacency Matrix of the last computed graph to a text file."""
        if not hasattr(self, 'graph_edges') or not self.graph_edges:
            print("Cannot save: compute a graph first (press 'c').")
            return
        ...
        try:
            ...
            print(f"Saved matrix data to {file_path}")
        except Exception as e:
            print(f"Error saving matrix data: {e}")

    def save_eigenvalues_pdf(self):
        """Export the Eigenvalue Spectrum to a PDF with properly labeled axes."""
        if self.eigvals is None:
            print("No eigenvalues to export. Compute spectrum first.")
            return
        ...
        # Highlight selected eigenvalue if any
    ...
    def save_graph_pdf(self):
        ...
        if self.graph_pos_x is None:
            print("Cannot export: state graph has not been computed. Press 'c' first.")
            return
        ...
        # 2. Draw nodes
        # Use current eigenvector scaling/colors if available
    ...
    def _draw_link_ellipse(self, ax, p1, p2, fill, edge):
        ...
        # Aspect ratio 5: width 1.0, height 0.2, thicker edges 2.0
    ...
    def save_coverage_pdf(self):
        ...
        fill_color, line_color = '#a9c0f6', '#0011ff'
        for d_idx in self.dimers:
            s1, s2 = self.lattice.edges[d_idx]
            p1, p2 = np.array(self.lattice.sites[s1]), np.array(self.lattice.sites[s2])
            dp = p2 - p1
            if abs(dp[0]) > 1:
                if p1[0] < p2[0]: 
        ...
        # File dialog
        file_path = self._get_save_path("Export Pattern as PDF", "dimer_covering.pdf")
        
        if not file_path:
            return

        fig.savefig(file_path, format='pdf', bbox_inches='tight')
        print(f"Exported covering to {file_path}")
    ...
    def on_press(self, event):
        ...
        if btn == 1:
            if edge_idx in self.dimers:
                ...
                # Add dimer if valid constraint visually
    ...
    def on_motion(self, event):
        if self.dragging_dimer is not None:
            ...
            if d <= 0.3 and edge_idx is not None:
                ...
                color = 'green' if self.is_valid_dimer(edge_idx) else 'red'
